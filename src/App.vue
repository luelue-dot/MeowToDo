...
useTauriListen(LISTEN_KEY.SHOW_WINDOW, async ({ payload }) => {
  ...
  // Handle { label, targetX, targetY } payload from task-bar
  const data = payload as Record<string, unknown>
  if (data.label && appWindow.label !== data.label) return

  // Position the window if target coordinates are provided
  if (data.targetX !== undefined && data.targetY !== undefined) {
    const appWindow = getCurrentWebviewWindow()
    await appWindow.setPosition({
      x: Math.round(Number(data.targetX)),
      y: Math.round(Number(data.targetY)),
    })
  }
