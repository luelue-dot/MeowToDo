...
/** Show the analytics window via the Rust backend. Falls back to event if unavailable. */
export async function showWindowByLabel(label: string) {
  if (label === 'analytics') {
    try {
      await invoke('show_analytics_window')
      return
    } catch (e) {
      console.warn('show_analytics_window failed:', e)
    }
  }
  // Fallback: emit event for other windows
  showWindow(label as WindowLabel)
}
