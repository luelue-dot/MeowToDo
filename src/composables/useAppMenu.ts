...
import { showWindow, showWindowByLabel } from '@/plugins/window'
...
export function useAppMenu() {
  ...
  const getBaseMenu = async () => {
    return await Promise.all([
      ...
      Submenu.new({
        text: t('composables.useAppMenu.labels.taskManager'),
        items: await Promise.all([
          MenuItem.new({
            text: t('composables.useAppMenu.labels.createTask'),
            action: () => {
              showWindow(WINDOW_LABEL.TODO)
            },
          }),
          MenuItem.new({
            text: t('composables.useAppMenu.labels.todayTasks'),
            action: () => showWindow(WINDOW_LABEL.TODO),
          }),
          MenuItem.new({
            text: t('composables.useAppMenu.labels.historyTasks'),
            action: () => showWindow(WINDOW_LABEL.TODO),
          }),
          MenuItem.new({
            text: t('composables.useAppMenu.labels.dailyReport'),
            action: () => showWindow(WINDOW_LABEL.DAILY_REPORT),
          }),
          MenuItem.new({
            text: t('composables.useAppMenu.labels.analytics'),
            action: () => showWindowByLabel(WINDOW_LABEL.ANALYTICS),
          }),
        ]),
      }),
      MenuItem.new({
